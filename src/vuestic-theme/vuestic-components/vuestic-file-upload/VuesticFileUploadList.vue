<template>
  <div class="vuestic-file-upload" :class="`vuestic-file-upload-${type}`">
    ...
    <template v-if="type === 'gallery'">
      <div class="row">
        <div class="flex xl2 lg3 sm4" v-for="(file, index) in filesList" :key="file.name">
          <vuestic-file-upload-gallery-item :file="file" @remove="$emit('remove', index)"/>
        </div>
      </div>
    </template>
    ...
    />
    <vuestic-file-upload-image
      v-if="type === 'image' && filesList.length"
      :file="filesList[filesList.length - 1]"
      @remove="$emit('remove-single')"
    />
  </div>
...
import VuesticFileUploadImage from './VuesticFileUploadImage';

export default {
  name: 'vuestic-file-upload-list',
  components: {
    VuesticFileUploadListItem,
    VuesticFileUploadGalleryItem,
    VuesticFileUploadSingleItem,
    VuesticFileUploadImage,
  },
  props: {
    type: {
      type: String,
