<template>
  <div class="vuestic-tabs">
    <div>
      <nav class="nav nav-pills va-row" :class="{'no-top-padding': noTopPadding}">
        <div
          class="nav-item col"
          ...
          :key="name"
        >
          <span class="nav-link"><h5>{{$t(name)}}</h5></span>
        </div>
      </nav>
      <div class="track">
    ...
    <div class="tab-content">
      <div
        ...
        :key="name"
      >
        <slot v-if="currentActive === name" :name="name"></slot>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    this.currentActive = this.currentTabProp || this.names[0];
  },
  methods: {
    setActive(name) {
  ...
  data() {
    return {
      currentActive: this.currentTabProp || this.names[0],
    };
  },
};
</script>
